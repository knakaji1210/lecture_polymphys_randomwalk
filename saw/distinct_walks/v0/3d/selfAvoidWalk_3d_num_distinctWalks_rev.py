# Calculation to necessary steps to list up total number of distinct 3d self-avoiding walks
...
while (i < len(n_list)) and (diff_numDistinctWalks > 0):
    n = n_list[i]
    distinctWalks_list = saws.saw3dDistinctWalks(N, n)
...
fig_title = "Number of Trials (3d)"
result_text1 = "$N$ = {}".format(N)
result_text2 = "$n_{{walks}}$ = {0:.0f}".format(numDistinctWalks)
...
savefile = "./png/selfAvoidingWalk_3d_numberTrials_N{0}.png".format(N)
