...
# distinctWalks_1d_N1_x.txtとdistinctWalks_N1_1d_y.txtを用意する必要がある（N0をスタートにすると動かない）

import numpy as np

def saw1dDistinctWalks(N):                                     # Degree of polymerizationが唯一の変数
...
# Function to obtain distinct 1d Random Walks of (n+1) steps from n steps
    ...
    export_file_x = "./data/distinctWalks_1d_N{0}_x.txt".format(fileNum+1)  # 次のステップのcsvファイルの名前
    export_file_y = "./data/distinctWalks_1d_N{0}_y.txt".format(fileNum+1)
