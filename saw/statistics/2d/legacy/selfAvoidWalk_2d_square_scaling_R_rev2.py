# Scaling of 2d Self-Avoiding Walk (square lattice model)
...
#log_list = np.logspace(1, ex, 2*ex+1, base=10)
log_list = np.logspace(0, ex, 6*ex+ex+1, base=10) # N=700（正確には701）をやるための一時的措置
#print(log_list)
log_list = log_list[:-1]    # N=700（正確には464）をやるための一時的措置
print(log_list)
...
fig_title = "Scaling of $R$ for 2-d Self-Avoiding Walk (square lattice model)"
...
savefile = "./png/selfAvoidingWalk_2d_square_scaling_R_N{0:.0f}M{1:.0f}.png".format(ex, logM)
