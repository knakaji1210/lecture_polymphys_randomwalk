...
# del x_list[-(len(x_list)-len(totalNum_list)):]
...
logTotalNum = log2(numDistinctWalks)
n_walk_min = n_list[-1]
x_min = ceil(x_list[-1])
...
fig_title = "Number of Trials (1d)"
result_text1 = "$N$ = {}".format(N)
result_text2 = "$n_{{walks}}$ = {0:.0f} = 2$^{{{1:.0f}}}$".format(numDistinctWalks, logTotalNum)
...
savefile = "./png/randomWalk_1d_numberTrials_N{0}.png".format(N)
