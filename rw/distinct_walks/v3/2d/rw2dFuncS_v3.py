...
def rw2dDistinctWalks(N):                                       # Degree of polymerizationが唯一の変数
...
# Function to obtain distinct 2d Random Walks of (n+1) steps from n steps
    ...
    import_file_x = "./data/distinctWalks_2d_N{0}_x.csv".format(file_num)    # 読み込むcsvファイルの名前  
    import_file_y = "./data/distinctWalks_2d_N{0}_y.csv".format(file_num)
    export_file_x = "./data/distinctWalks_2d_N{0}_x.csv".format(file_num+1)  # 次のステップのcsvファイルの名前
    export_file_y = "./data/distinctWalks_2d_N{0}_y.csv".format(file_num+1)
