...
# distinctWalks_2d_N1_x.txtとdistinctWalks_2d_N1_y.txtを用意する必要がある（N0をスタートにすると動かない）
# csvを使ったv3よりも圧倒的に速い
# ファイル分割に対応させたバージョン

import numpy as np

def rw3dDistinctWalks(N,sp):                                      # Degree of polymerizationが唯一の変数
...
# Function to obtain distinct 2d Random Walks of (n+1) steps from n steps
    ...
    import_file_x = "./data/distinctWalks_2d_N{0}_x_sp{1}.txt".format(fileNum,sp)    # 読み込むcsvファイルの名前  
    import_file_y = "./data/distinctWalks_2d_N{0}_y_sp{1}.txt".format(fileNum,sp)
    export_file_x = "./data/distinctWalks_2d_N{0}_x_sp{1}.txt".format(fileNum+1,sp)  # 次のステップのcsvファイルの名前
    export_file_y = "./data/distinctWalks_2d_N{0}_y_sp{1}.txt".format(fileNum+1,sp)
