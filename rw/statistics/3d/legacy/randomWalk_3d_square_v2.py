...
ax1_title = "3D Random Walk (square) ($N$ = $10^{0:.0f}$, $M$ = $10^{1:.0f}$)".format(logN, logM)
...
param, cov = curve_fit(fitFunc, hist_RX, hist_RY, p0=[N, 1e-4])
...
fit_RY = []
for x in fit_RX :
    fit_RY.append( param[1]*x*x*np.exp(-3*x*x/param[0]/2))
...
fig.text(0.80, 0.86, resultText_Rfit)

savefile = "./png/randomWalk_3d_square_N{0:.0f}M{1:.0f}.png".format(logN, logM)
