...
ax1_title = "2-d Random Walk (square) ($N$ = $10^{0:.0f}$, $M$ = $10^{1:.0f}$)".format(logN, logM)
ax2_title = "Distribution of $R$ = <$r^2$>"
...
hist_RX, hist_RY, histInfo_R = hist.histDraw(fig, 2, 2, 2, ax2_title, ax2_x, "orange", r_list)
...
hist_XX, hist_XY, histInfo_X = hist.histDraw(fig, 2, 2, 3, ax3_title, ax3_x, "green", xt_list)
...
hist_YX, hist_YY, histInfo_Y = hist.histDraw(fig, 2, 2, 4, ax4_title, ax4_x, "green", yt_list)
...
fig.text(0.15, 0.61, resultText_r)
fig.text(0.15, 0.59, resultText_rmean)
fig.text(0.15, 0.57, resultText_R)
fig.text(0.36, 0.43, resultText_xmean)
fig.text(0.36, 0.41, resultText_xstd)
fig.text(0.78, 0.43, resultText_ymean)
fig.text(0.78, 0.41, resultText_ystd)

fig.text(0.78, 0.85, resultText_Rfit)

savetext = "./png/randomWalk_2d_square_N{0:.0f}M{1:.0f}.png".format(logN, logM)

fig.savefig(savetext, dpi=fig.dpi)
