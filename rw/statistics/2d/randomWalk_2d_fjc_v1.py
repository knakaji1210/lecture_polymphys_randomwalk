...
import rw2dFuncS_fjc as rws
import rw2dFuncM_fjc as rwm
...
# 以下のように修正（あくまでも絶対値として大きい数値で範囲指定したいから）
x_max = np.max([ np.abs(x) for x in xt_list ])
y_max = np.max([ np.abs(y) for y in yt_list ])
