# 2d Random Walk (freely-jointed chain model)

import numpy as np
from math import *
from scipy.stats import norm
from scipy.optimize import curve_fit
import rw2dFunc_fjc_v2 as rw
...
ax1_title = "2D Random Walk (FJC chain) ($N$ = $10^{0:.0f}$, $M$ = $10^{1:.0f}$)".format(logN, logM)
...
np.savetxt("./data/hist_YX_N{0:.0f}M{1:.0f}.txt".format(logN, logM), hist_YX)
np.savetxt("./data/hist_YY_N{0:.0f}M{1:.0f}.txt".format(logN, logM), hist_YY)
np.savetxt("./data/histInfo_Y_N{0:.0f}M{1:.0f}.txt".format(logN, logM), histInfo_Y)
...
savefile = "./png/randomWalk_2d_fjc_N{0:.0f}M{1:.0f}.png".format(logN, logM)
